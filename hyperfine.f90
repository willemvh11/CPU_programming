program lens
  ...
  gammain=5.8908d+7
  gammaas=4.595d+7
  call blende(a,in,maxin,as,maxas,rmax)
  allocate(hypin(maxin))
  allocate(hypas(maxas))
  open(1,file="hypin.txt")
  open(2,file="hypas.txt")
  do n=1,maxin
  ...
  print*,'maxin = ', maxin
  print*,'maxas = ', maxas
  print*,'ni = ', maxin+maxas
end program lens

subroutine blende(a,in,k,as,s,rmax)
  implicit none
  real*8, dimension(3,500000) :: p,h,in,as
  real*8 :: x,y,z,a,r,rmax
  integer :: n,j,s,k
  n=1
  z=0.0d+0
  do while (z<=2.0d+0*rmax)
    x=0.0d+0
    do while (x<=2.0d+0*rmax)
      y=0.0d+0
      do while (y<=2.0d+0*rmax)
        p(:,n)=[x,y,z]
  ...
  do j= 1,n-1
    ...
    if ( r<=rmax .and. p(3,j)>=0.0d+0 ) then
      in(1,k)=p(1,j)
      in(2,k)=p(2,j)
      in(3,k)=p(3,j)
      k=k+1
    ...
    if ( r<=rmax .and. h(3,j)>=0.0d+0 ) then
      as(1,s)=h(1,j)
      as(2,s)=h(2,j)
      as(3,s)=h(3,j)
      write(1,*)as(:,s)
